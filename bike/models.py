...
class Bike(models.Model):
    ...
    state = FSMField(
        default=BikeState.RECEIVED, choices=BikeState.CHOICES, protected=True
    )
    claimed_by = models.ForeignKey(
        Member, on_delete=models.SET_NULL, null=True, related_name="claimed_bike"
    )
    stolen = models.NullBooleanField()
    purchased_by = models.ForeignKey(
        Member, on_delete=models.SET_NULL, null=True, related_name="purchased_bike"
    )
    ...
    def __validate_required(self, required_fields: list) -> dict:
        ...
        return {
            "field_errors": {
                pair[0]: self.field_errors_required[pair[0]]
    ...
    def can_assessed(self) -> bool:
        ...
        """
        required_fields = ["colour", "make", "size", "source", "price"]
    ...
    def can_available(self):
        ...
        if (
            self.claimed_by
            and self.last_worked_on
            and self.last_worked_on > timezone.now() - timedelta(weeks=4)
        ):
    ...
    @transition(
        field=state,
        source=[BikeState.ASSESSED, BikeState.CLAIMED],
        target=BikeState.AVAILABLE,
    )
    def available(self):
    ...
    @transition(
        field=state, source=[BikeState.AVAILABLE], target=BikeState.PURCHASED,
    )
    ...
    @transition(
        field=state,
        source=[BikeState.ASSESSED, BikeState.AVAILABLE],
    ...
    def available_states(self):
        ...
        states = {
            ...
            if state_transition.name not in omit
        }
