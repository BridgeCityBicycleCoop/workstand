...
class TestBikeApi(test.TestCase):
    ...
    #         "source": Bike.COS_BIKE_DIVERSION_PILOT,
    #         "donated_by": "Greg",
    #         "donated_at": timezone.datetime(2017, 1, 1),
    #     }
    ...
    def test_claim_cannot_transition_wrong_state(self):
        ...
        client.force_authenticate(user=self.user, token="blah")
        result = client.put(
            f"/api/v1/bikes/{bike.id}/claim/", data={"member": member.id}, format="json"
        )
    ...
    def test_claim_cannot_transition_claimed(self):
        ...
        client.force_authenticate(user=self.user, token="blah")
        result = client.put(
            "/api/v1/bikes/{bike_id}/claim/".format(bike_id=bike.id),
    ...
    def test_purchase_can_transition_from_available(self):
        ...
        client.force_authenticate(user=self.user, token="blah")
        result = client.put(
            f"/api/v1/bikes/{bike.id}/purchase/",
            data={"member": member.id},
            format="json",
        )
    ...
    def test_purchase_cannot_transition_when_claimed(self):
        ...
        client.force_authenticate(user=self.user, token="blah")
        result = client.put(
            "/api/v1/bikes/{bike_id}/purchase/".format(bike_id=bike.id),
            data={"member": member.id},
            format="json",
        )
