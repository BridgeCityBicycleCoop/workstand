...
class MembershipForm(ModelForm):
    ...
    safe_space = BooleanField(
        required=True, widget=CheckboxInput(attrs={"class": "mdl-checkbox__input"})
    )
    respect_community = BooleanField(
        required=True, widget=CheckboxInput(attrs={"class": "mdl-checkbox__input"})
    )
    give_back = BooleanField(
        required=True, widget=CheckboxInput(attrs={"class": "mdl-checkbox__input"})
    )
    respect_shop = BooleanField(
        required=True, widget=CheckboxInput(attrs={"class": "mdl-checkbox__input"})
    )
    ...
    class Meta:
        ...
        widgets = {
            "self_identification": RadioSelect(
                choices=self_ident_choices, attrs={"class": "mdl-radio__button"}
            ),
            "gender": RadioSelect(
                choices=gender_choices, attrs={"class": "mdl-radio__button"}
            ),
