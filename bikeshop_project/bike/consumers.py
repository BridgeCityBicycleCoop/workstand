...
def _is_stolen(serial: str) -> Optional[bool]:
    ...
    found_records = (
        r"^WE HAVE A RECORD ON FILE THAT MATCHES THE IDENTIFIERS THAT YOU PROVIDED.+$"
    )
...
def check_cpic(message: Dict[str, Union[str, int]]) -> None:
    ...
    if stolen:
        ...
        logger.error(
            f'check_epic: Unable to check CPIC records with serial number: {message["serial_number"]}.'
        )
        return
    else:
        logging.info(
